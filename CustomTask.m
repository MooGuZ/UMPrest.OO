classdef CustomTask < Task
    methods
        function run(obj, nepoch, batchPerEpoch, batchsize, validsize)
            ...
            for epoch = 1 : nepoch
                ...
                if obj.inWorkSpace
                    obj.wspace.iteration = obj.iteration;
                end
                obj.validate(validset);
                obj.save();
            end
        ...
        function modeldump = save(obj)
            if obj.saveCounter < obj.saveInterval
                obj.saveCounter = obj.saveCounter + 1;
            else
                if obj.inWorkSpace
                    modeldump = obj.wspace.dump();
                else
    ...
    methods
        function obj = CustomTask(taskid, taskdir, model, dataset, objective, priors, varargin)
            ...
            % setup save frequence
            obj.saveInterval = conf.pop('saveInterval', inf);
            % private parameters
            obj.latestSave  = [];
            obj.saveCounter = 1;
            % WorkSpace related
            obj.inWorkSpace = false;
            obj.wspace = [];
        end

        function obj = setWorkSpace(obj, wspace)
            obj.iteration = wspace.iteration;
            obj.inWorkSpace = true;
            obj.wspace = wspace;
        end
    end
    ...
    properties (SetAccess = protected, Hidden)
        latestSave, saveInterval, saveCounter
        wspace, inWorkSpace
    end
    ...
    methods
        ...
        function set.saveInterval(obj, value)
            assert(value >= 0 && (MathLib.isinteger(value) || isinf(value)), 'ILLEGAL ASSIGNMENT');
            obj.saveInterval = value;
        end
