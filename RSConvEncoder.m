...
classdef RSConvEncoder < RecurrentUnit
    methods
        function obj = RSConvEncoder(framesize, ...
    ...
    methods
        function unitdump = dump(obj)
            unitdump = {'RSConvEncoder', obj.framesize, ...
    ...
    methods (Static)
        ...
        function debug(niter, probScale, batchsize, validsize)
            ...
            nframes = nbase;
            % reference model
            refer = RSConvEncoder.randinit(frmsize, nlayerFrm, basesize, nbase, [2,2]);
            cellfun(@(hp) hp.set(randn(size(hp))), refer.hparam);
            refer.refresh();
            % approximate model
            model = RSConvEncoder.randinit(frmsize, nlayerFrm, basesize, nbase, [2,2]);
            % create dataset
